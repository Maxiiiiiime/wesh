...
const productsList = ref( [
  ...
  const productsLike = ref([])

  const productsCard = ref([])

  const addToCart = (produit) => {
    productsCard.value.push({...produit})
  }

  const addToLike = (produit) => {
    let index = (productsLike.value.findIndex(e => e.id == produit.id))
    
    if (index < 0) {
       productsLike.value.push({...produit})
    }
    else {
      productsLike.value.splice(index,1)
    }
    ...
    <div class="home-container">
      ...
      <div class="card-cart-container">
        <div class="card-container">
            <Product @addToLike="addToLike" @addToCart="addToCart" v-for="(produit, id) in productsList" :key="id" :produit="produit"></Product>
        </div>
      </div>
    </div>
  </main>
